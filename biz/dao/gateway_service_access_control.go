...
func (t *AccessControl) Find(ctx context.Context, db *gorm.DB, search *AccessControl) (*AccessControl, error) {
	model := &AccessControl{}
	err := db.WithContext(ctx).Where(search).Find(model).Error
	return model, err
}

func (t *AccessControl) Save(ctx context.Context, db *gorm.DB) error {
...
func (t *AccessControl) ListBYServiceID(ctx context.Context, tx *gorm.DB, serviceID int64) ([]AccessControl, int64, error) {
