...
func (t *HttpRule) Find(ctx context.Context, db *gorm.DB, search *HttpRule) (*HttpRule, error) {
	model := &HttpRule{}
	err := db.WithContext(ctx).Where(search).Find(model).Error
	return model, err
}

func (t *HttpRule) Save(ctx context.Context, db *gorm.DB) error {
...
func (t *HttpRule) ListByServiceID(ctx context.Context, db *gorm.DB, serviceID int64) ([]HttpRule, int64, error) {
	var list []HttpRule
	var count int64
