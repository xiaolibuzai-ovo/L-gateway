...
func (t *LoadBalance) Find(ctx context.Context, db *gorm.DB, search *LoadBalance) (*LoadBalance, error) {
	model := &LoadBalance{}
	err := db.WithContext(ctx).Where(search).Find(model).Error
	return model, err
}

func (t *LoadBalance) Save(ctx context.Context, db *gorm.DB) error {
